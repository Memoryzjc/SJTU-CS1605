...
// Function to carry out attack and return damage
...
void carryOutAction(Slime* playerSlime, Slime* enemySlime,
    ...
    // Priority: Change > Potion > Skill
    if (enemyChoice == "Change" || enemyChoice == "revivalPotion" || enemyChoice == "atkPotion") {
...
std::vector<Slime*> battleStage1_2(std::vector<Slime*> & playerSlimes,
                                   ...
                                   int & revivalPotion, int & atkPotion) {
    assert(!playerSlime->isDead() && !enemySlime->isDead());
    ...
    if (enemySlimeNum == 1) {
        if (atkPotion) {
            if (!isSlimeRestrain(enemySlime, playerSlime) && !enemySlime->getAtkPotionEffect()) {
                enemySlime->setAtkPotionEffect(true);
                atkPotion--;
                enemyChoice = "atkPotion";
            } else {
    ...
    } else if (enemySlimeNum == 2) {
        if (revivalPotion) {
            // Use the revival potion or not
            for (auto & slime : enemySlimes) {
                if (slime->isDead()) {
                    slime->setHP(slime->getMaxHP() / 2);
                    revivalPotion--;
                    enemyChoice = "revivalPotion";
                    revivalSlime = slime;
                    break;
                }
            }
        } else if (atkPotion) {
            if (!isSlimeRestrain(enemySlime, playerSlime)
                && !enemySlime->getAtkPotionEffect()) {
                enemySlime->setAtkPotionEffect(true);
                atkPotion--;
                enemyChoice = "atkPotion";
            } else {
                // Can't use the Attack Potion
                // Change the slime or use the skill
                if (enemySlime->getAtkPotionEffect()) {
                    enemyAction = (isSlimeRestrain(playerSlime, enemySlime)) ? 2 : 1;
                    enemyChoice = enemySlime->getSkill()[enemyAction - 1].getName();
                } else {
                    if (findRestrainSlime(enemySlimes, playerSlime)
                        && enemySlime != findRestrainSlime(enemySlimes, playerSlime)) {
                        enemySlime->setAtkPotionEffect(false);
                        enemySlime = findRestrainSlime(enemySlimes, playerSlime);
                        enemyChoice = "Change";
    ...
    if (enemyChoice == "revivalPotion") {
        std::cout << "Enemy uses Revival Potion on " << revivalSlime->getName() << std::endl;
    } else if (enemyChoice == "atkPotion") {
        std::cout << "Enemy uses Attack Potion on " << enemySlime->getName()  << std::endl;
    } else if (enemyChoice == "Change") {
...
void play(std::vector<Slime*> & playerSlimes, std::vector<Slime*> & enemySlimes) {
    ...
    // Battle begins
    int round = 1, revivalPotion = 1, atkPotion = 2;
    std::vector<Slime*> battleSlimes;
    while (true) {
        std::cout << "Round " << round << std::endl;
        assert(playerBattleSlime && enemyBattleSlime);
        // Stage 1 & 2
        battleSlimes = battleStage1_2(playerSlimes, enemySlimes,
                                      playerBattleSlime, enemyBattleSlime,
                                      revivalPotion, atkPotion);
        ...
        if (enemyBattleSlime->isDead()) {
            ...
            enemyBattleSlime->setAtkPotionEffect(false);
